...
sce <- as.SingleCellExperiment(S.combined, assay = "RNA")
plotUMAP(sce, colour_by = "SIdent")
assays(sce)
dim(counts(sce))
counts(sce)[1:6,1:6]
dim(colData(sce))
...
colData(sce) %>% as.data.frame %>% 
  transmute(Biorep, Status, ClusterID = SIdent, 
...
S.agg <- aggregate.Matrix(t(counts(sce)), groupings = groups, fun = "sum")
S.agg[1:6,1:6]

# Resplitting matrix by ClusterID and transform

S.agg <- split.data.frame(S.agg, factor(CIDs)) %>%
...
for(clst in names(S.agg)) {
  print(clst)
  dat <- as.matrix(S.agg[[clst]])
...
clipr::write_clip(tt[, list(n_fdr = sum(FDR < 0.01)), by = list(cluster)][order(cluster)])

# Retic vs Normo ----

S.InfvsNaive <- split(tt, f = tt$cluster)

# Save files

saveRDS(S.InfvsNaive, paste0(output.filepath,"S.InfvsNaive.rds"))
